...
@section('content')
    <section class="section-0 lazy d-flex bg-image-style dark align-items-center " class=""
        data-bg="{{ asset('frontend') }}/assets/images/banner5.jpg">
        <div class="container">
            <div class="row">
                                ...
                                @foreach ($isfeatured as $isfeatured)
                                    <div class="col-md-4">
                                        <div class="card border-0 p-3 shadow mb-4">
                                            <div class="card-body">
                                                <h3 class="border-0 fs-5 pb-2 mb-0">{{ $isfeatured->title }}</h3>
                                                <p>{{ Str::words($isfeatured->description, 10) }}</p> {{-- only 10 words print --}}

                                                <div class="bg-light p-3 border">
                                                    <p class="mb-0">
                                                        <span class="fw-bolder"><i class="fa fa-map-marker"></i></span>
                                                        <span class="ps-1">{{ $isfeatured->location }}</span>
                                                    </p>
                                                    <p class="mb-0">
                                                        <span class="fw-bolder"><i class="fa fa-clock-o"></i></span>
                                                        <span class="ps-1">{{ $isfeatured->jobType->name }}</span>
                                                    </p>
                                                    @if (!is_null($isfeatured->salary))
                                                        <p class="mb-0">
                                                            <span class="fw-bolder"><i class="fa fa-usd"></i></span>
                                                            <span class="ps-1">{{ $isfeatured->salary }}</span>
                                                        </p>
                                                    @endif
                                                </div>
                            ...
                            @if ($latestjobs->isNotEmpty())
                                @foreach ($latestjobs as $latestjobs)
                                    <div class="col-md-4">
                                        <div class="card border-0 p-3 shadow mb-4">
                                            <div class="card-body">
                                                <h3 class="border-0 fs-5 pb-2 mb-0">{{ $latestjobs->title }}</h3>
                                                <p>{{ Str::words($latestjobs->description, 10) }}</p> {{-- only 10 words print --}}
